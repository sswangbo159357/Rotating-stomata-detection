...
class ctDataset(data.Dataset):
    ...
    mean = np.array([0.7402756, 0.56988525, 0.45618063], dtype=np.float32).reshape(1, 1, 3)
    std = np.array([0.30838934, 0.25536352, 0.22110835], dtype=np.float32).reshape(1, 1, 3)

    def __init__(self, data_dir='data', split='train'):
        self.data_dir = os.path.join(data_dir, 'Stomata')
    ...
    def __getitem__(self, index):
        ...
        conduct = np.zeros((self.max_objs, 1), dtype=np.float32)
...
def coco_box_to_bbox(box):
    bbox = np.array([box[0] - box[1] / 2, box[2] - box[3] / 2, box[1] + box[3] / 2, box[2] + box[3] / 2],
                    dtype=np.float32)
    ang = float(box[4])
    return bbox, ang
